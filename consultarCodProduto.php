      ...
      <div class="container-fluid bg-light  text-center ">
        <div class="row">
          <?php
            ...
            $codProd = isset($_POST['txtCodProd']) ? trim($_POST['txtCodProd']) : "";


            if (!empty($codProd)) {
              
              $stmt = $pdo->prepare("SELECT ID_PRODUTOS, NOME, COD_PRODUTO, PRECO, QUANTIDADE, DATA_VALIDADE, COD_BARRAS, LOCAL_ARMAZENADO, DESCRICAO, USUARIO, DATA_CADASTRO FROM PRODUTOS WHERE COD_PRODUTO LIKE :CodProd ORDER BY NOME");
              ...
              if ($count != 0) {
                ...
                foreach ($stmt as $row ) {
                  ...
                  <td>'.number_format((float) $row["PRECO"], 2, ',', '.').' R$'.'</td>
                  ...
                  if ($_SESSION['tipo'] == 1) 
                  {
                    ...
                    // Botão Editar
                    echo '<td colspan="2"> <a class="btn btn-sm btn-success text-white m-2" href="editarProduto.php?idProduto='.$idProdutos = $row['ID_PRODUTOS'].'"><i class="fas fa-pen"></i>&nbsp&nbspEditar</a>';
